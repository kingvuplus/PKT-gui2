...
class InfoBarExtensions:
	...
	def __init__(self):
		...
		self["InstantExtensionsActions"] = HelpableActionMap(self, "InfobarExtensions", {'extensions': (self.showExtensionSelection, _('Show extensions...')),
				{
					"extensions_PKT': (self.callFantasticGreen, _('PKT Blue Panel...')),
					...
					"openEmuMan": (self.callEmuMan, _("Show EmuManager...")),
					"openMediaPlayer": (self.callMediaPlayer, _("Show MediaPlayer...")),
					"openIPTVPlayer": (self.callIPTVPlayer, _("Show IPTVPlayer...")),
					"openPowerTimerEdit": (self.callPowerTimerEdit, _("Show PowerTimerEdit...")),
	...
	def callFantasticGreen(self):
            from Plugins.Extensions.PKT.plugin import BPanel
            self.session.open(BPanel)

        def callEmuMan(self):
            from Plugins.Extensions.EMUmanager.plugin import ScreenEmu
            self.session.open(ScreenEmu)

        def callMediaPlayer(self):
            from Plugins.Extensions.MediaPlayer.plugin import MediaPlayer
            self.session.open(MediaPlayer)

        def callIPTVPlayer(self):
            if os.path.exists('/usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/plugin.pyo'):
                from Plugins.Extensions.IPTVPlayer.plugin import *
                self.session.open(IPTVPlayerWidget)
            else:
                self.session.open(MessageBox, _('The IPTVPlayer plugin is not installed!\nPlease install it.'), type=MessageBox.TYPE_INFO, timeout=10)

        def callPowerTimerEdit(self):
